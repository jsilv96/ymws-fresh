...
function initCookieBanner() {
  ...
  const PREFS_KEY = "rm_cookie_prefs";
  ...
  banner.setAttribute("aria-labelledby", "cookie-heading");
  banner.setAttribute("aria-describedby", "cookie-desc");
  banner.innerHTML = `
    <div class="cookie-main">
      <div class="cookie-text">
        <strong id="cookie-heading"><i class="fa-solid fa-cookie-bite" aria-hidden="true"></i> We value your privacy</strong>
        <p id="cookie-desc">We use cookies to improve your experience, analyze traffic, and serve tailored content. Choose which cookies you're comfortable with. <a href="cookies.html">Learn more</a>.</p>
      </div>
      <div class="cookie-actions">
        <button class="btn btn-text decline-all" type="button">Reject All</button>
        <button class="btn btn-text customize" type="button">Customize</button>
        <button class="btn btn-primary accept-all" type="button">Accept All</button>
        <button class="btn btn-text cancel-prefs" type="button">Cancel</button>
        <button class="btn btn-primary save-prefs" type="button">Save Preferences</button>
      </div>
    </div>
    <div class="cookie-prefs">
      <div class="cookie-category">
        <div>
          <h5>Essential</h5>
          <p>Required for core site features like cart and checkout. Always on.</p>
        </div>
      ...
      <div class="cookie-category">
        <div>
          <h5 id="cookie-pref-prefs">Preferences</h5>
          <p>Remember your choices like language and location.</p>
        </div>
        <label class="cookie-switch">
          <input type="checkbox" data-cat="preferences" aria-labelledby="cookie-pref-prefs" checked />
          <span class="cookie-switch-track"></span>
        </label>
      </div>
      <div class="cookie-category">
        <div>
          <h5 id="cookie-pref-analytics">Analytics</h5>
          <p>Help us understand how people use our site.</p>
        </div>
        <label class="cookie-switch">
          <input type="checkbox" data-cat="analytics" aria-labelledby="cookie-pref-analytics" />
          <span class="cookie-switch-track"></span>
        </label>
      </div>
      <div class="cookie-category">
        <div>
          <h5 id="cookie-pref-marketing">Marketing</h5>
          <p>Serve personalized promotions across other websites.</p>
        </div>
        <label class="cookie-switch">
          <input type="checkbox" data-cat="marketing" aria-labelledby="cookie-pref-marketing" />
          <span class="cookie-switch-track"></span>
        </label>
      </div>
    </div>
  ...
  const toggles = banner.querySelectorAll("[data-cat]");
  const stored = JSON.parse(localStorage.getItem(PREFS_KEY) || "{}");
  toggles.forEach((t) => {
    if (stored[t.dataset.cat] !== undefined) t.checked = stored[t.dataset.cat];
  });
  ...
  banner.querySelector(".cancel-prefs").addEventListener("click", () => {
    banner.classList.remove("expanded");
  });

  banner.querySelector(".save-prefs").addEventListener("click", () => {
    const prefs = {};
...
function initSignupForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (msg) {
      msg.textContent = `Confirmed. We'll send promos to ${email.value}.`;
